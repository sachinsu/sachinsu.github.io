...
draft: true
tags: [Presto, SQL, Database, Parquet]
---
...
For these cases, One may consider [Presto](https://prestodb.io/) or [Trino](https://trino.io). At it's core, Presto translates SQL queries into whatever query language is necessary to access the underlying storage medium. Storage medium could be a Elasticsearch cluster, or a set of Parquet files on HDFS, or a relational database.
...
Presto provides [connectors](https://trino.io/docs/current/connector.html) for variety of data sources like ElasticSearch, Relational databases (PostgreSQL, Oracle etc.), Local file and on. 
...
Below is apt representation of how Presto works (Coutesy: prestodb.io)
...
* Given that Presto has no storage on its own, how can one perform Capacity planning given the expected workflow. 
